module Reviews
  class Index
    ...
    def call
      relation = Reviews::Query.new(
        scope: Review.includes(:location, :content_creator).references(:location, :content_creator),
        query: @search_params[:query],
        sort_by: @search_params[:sort_by],
        bounds: @search_params.slice(:sw_lat, :sw_lng, :ne_lat, :ne_lng)
      ).call

      total_count = relation.count

      {
        reviews: relation.limit(page_size).offset(offset),
        meta: {
          page: page,
          page_size: page_size,
